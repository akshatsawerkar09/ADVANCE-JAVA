...
public class AuthenticationServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		 ...
		 */
		try {
			// get servlet specific init params from servelt config
			ServletConfig config = getServletConfig();
			customerDao = new CustomerDaoImpl(config.getInitParameter("drvr"), config.getInitParameter("url"),
					config.getInitParameter("user"), config.getInitParameter("pwd")); // we cannot use throws Exception
																						// because overiding form of
																						// method
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (customer == null) {
				...
				// pw.flush(); //java.lang.IllegalStateException: Cannot forward after response
				// has been committed
